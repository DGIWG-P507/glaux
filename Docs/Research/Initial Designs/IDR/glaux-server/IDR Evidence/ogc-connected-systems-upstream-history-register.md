...
**Mutable `part3-working-draft` snapshot checked:** September 15, 2026, commit [`6f529a15`](https://github.com/opengeospatial/ogcapi-connected-systems/commit/6f529a15bfa63259febc3620378d3e5a06305333)<br>
...
`Open` and `Closed` below are only the states observed through the latest bounded refresh on September 15, 2026. A closed issue is not proof that its proposed change was implemented; issue [#64](https://github.com/opengeospatial/ogcapi-connected-systems/issues/64) is a concrete counterexample.
...
| Part 1 bundled OAS 3.1 | [`ogcapi-connectedsystems-1.bundled.oas31.yaml`](https://github.com/opengeospatial/ogcapi-connected-systems/releases/download/v1.0.0/ogcapi-connectedsystems-1.bundled.oas31.yaml), SHA-256 `69da631d5d05f01716381cca7b7ee6311402f2752a8fd79a9b72b663539555aa`. The release asset retains 32 relative `../examples/...` references and is not a standalone deployment/API-definition contract. The August 31 Redocly CLI 2.43.2 probe reported 44 errors/299 warnings; static generation terminated with exit 134. | Inspect and mechanically compare with the tagged modular Part 1 OAS; record variant-specific defects, bound recursive-schema tooling, and do not publish it unchanged as an implementation definition. |
| Part 2 bundled OAS 3.1 | [`ogcapi-connectedsystems-2.bundled.oas31.yaml`](https://github.com/opengeospatial/ogcapi-connected-systems/releases/download/v1.0.0/ogcapi-connectedsystems-2.bundled.oas31.yaml), SHA-256 `86ed005f9e7cf176264d6deb72581a0b521a227cd7a198b6cb1bd32b39d83667`. The release asset retains 51 relative references—45 examples and six schemas—and is not a standalone deployment/API-definition contract. The August 31 Redocly CLI 2.43.2 lint probe ended in a maximum-call-stack failure; static generation terminated with exit 134. | Inspect and mechanically compare with the tagged modular Part 2 OAS; record variant-specific defects, bound recursive-schema tooling, and do not publish it unchanged as an implementation definition. |
| Published modular package equality | Part 1 ZIP SHA-256 `43fcab8fb079b153e1dc01559c9395a51e8fab0e7c87c709fdae6a28b1983f12`; Part 2 ZIP SHA-256 `02accc4dd11a197f029a9a65d6e9eb3724ef3e7dc8a7e6e82bc05504844100a9`. After CRLF/LF normalization, all 152 published Part 1 OAS files and all 159 published Part 2 OAS files equal their `v1.0.0` tagged counterparts. Each package substitutes a package README; Part 2 also carries five AsyncAPI files. | Treat the tag as reproducible source provenance for published modular OAS content while preserving package-specific inventories and hashes. |
| Tagged navigation inventory | The modular Part 1 OAS contains 20 paths and Part 2 contains 23. Part 2 omits the required Deployment-scoped stream routes, stream-to-Sampling-Feature/feature-of-interest routes, all Feasibility routes, and the canonical SystemEvent item, while retaining removed System History routes. | Use the approved standards plus a documented normative overlay for route/link planning; preserve OAS omissions and extras as test fixtures. |
...
| Part 3 and AsyncAPI | Part 2 assigns publish/subscribe binding to a future Part 3. A legacy AsyncAPI 2.6 support file is packaged with Part 2. The active [`part3-working-draft` tree](https://github.com/opengeospatial/ogcapi-connected-systems/tree/6f529a15bfa63259febc3620378d3e5a06305333/api/part3) at commit `6f529a15` contains draft Resource Events, Resource Data Messages, content-negotiation material, and Batch Resource Events as an optional Resource Events subclass under `/req/resource-events/batch-events`; it contains no AsyncAPI file or ATS. The September 1 commit changed the Batch class identifiers/inheritance and removed the generated HTML, but the source remains `swg-draft` and explicitly leaves the MQTT clause unwired and examples/relationship annexes missing. | The IDR-SRV-035 report in review recommends a disabled-by-default, version-pinned experimental Glaux profile rather than approved adoption: transport-neutral durable events, SSE first, then a local MQTT 5 binding for exact snapshot `6f529a15`, generated AsyncAPI 3.0, explicit deviations, and no OGC Part 3 conformance claim. Refresh and migrate through versioned adapters on material upstream change. |

## 4. Screening Coverage and Selection Rules

The initial pass screened **all 141 official issues** then present (60 open, 81 closed) and reviewed **all 58 pull requests** then and still present. The August 31, 2026 bounded refresh added new issue #200, producing **142 issues** (61 open, 81 closed) and unchanged pull-request totals (3 open, 52 merged, 3 closed unmerged). GitHub uses one number sequence for issues and pull requests, which explains gaps in the issue numbering. Of the 142 current issues, **119 are retained** and 23 are excluded as purely editorial, administrative, build/website work without artifact consequences, superseded discussion without added insight, or otherwise immaterial to server design.
...
| [#2](https://github.com/opengeospatial/ogcapi-connected-systems/issues/2) | Closed | Explains the split between generic Web links in `links[]` and named resource-model associations. No explicit linked implementation change. | PB | 010, 017 |
| [#4](https://github.com/opengeospatial/ogcapi-connected-systems/issues/4) | Open | Unadopted proposals would clarify direct links versus templates, registered relations, and removal of undefined service fields. | UP | 010, 017 |
...
| [#23](https://github.com/opengeospatial/ogcapi-connected-systems/issues/23) | Open | Encoding conformance classes may overstate write support; per-method/media/schema advertisement remains unresolved. | UP | 008, 009, 010A, 012, 013 |
...
| [#61](https://github.com/opengeospatial/ogcapi-connected-systems/issues/61) | Closed | System cascade deletes nested resources but removes the System link from an associated Deployment rather than deleting that Deployment; [PR #90](https://github.com/opengeospatial/ogcapi-connected-systems/pull/90). | PB | 013, 030 |
| [#62](https://github.com/opengeospatial/ogcapi-connected-systems/issues/62) | Closed | Local deployed-system links use the System `uniqueID`; external links may use the external URL; [PR #92](https://github.com/opengeospatial/ogcapi-connected-systems/pull/92). | PB | 010, 017 |
...
| [#68](https://github.com/opengeospatial/ogcapi-connected-systems/issues/68) | Open | Part 3 direction shifted from EDR Pub/Sub toward common OGC API Pub/Sub and AsyncAPI 3.0; no final publication. | PCD | 014H, 035 |
...
| [#77](https://github.com/opengeospatial/ogcapi-connected-systems/issues/77) | Open | The published release contains maintained OAS 3.1 bundles only; an earlier SWG intent to supply both OAS 3.0 and 3.1 remains unfulfilled and unresolved. | Mixed | 010A, 014 |
...
| [#87](https://github.com/opengeospatial/ogcapi-connected-systems/issues/87) | Closed | SWG chose JSON Schema 2020-12 to align with OAS 3.1; [PR #124](https://github.com/opengeospatial/ogcapi-connected-systems/pull/124) converted the repository. | PB | 010A, 023 |
| [#91](https://github.com/opengeospatial/ogcapi-connected-systems/issues/91) | Closed | [PR #138](https://github.com/opengeospatial/ogcapi-connected-systems/pull/138) removed claims that associations can be inline or by-reference because the actual model provides links/references. | PB | 010, 017 |
...
| [#104](https://github.com/opengeospatial/ogcapi-connected-systems/issues/104) | Open | Reviewer’s Guide maintenance rationale includes intentionally unspecified external encoding versions; no formal closure or linked PR. | PCD | 012, 014H, 035 |
...
| [#149](https://github.com/opengeospatial/ogcapi-connected-systems/issues/149) | Open | History resources were removed as too complex and `datetime` was the interim design, but Part 2 OAS still contains `/history`; future versioning alignment is unresolved. | Mixed | 010, 010A, 018 |
...
| [#189](https://github.com/opengeospatial/ogcapi-connected-systems/issues/189) | Open | Part 3 MQTT clause is an unwired stub lacking normative version, topics, operations, QoS/session, retained-message, event, and ATS behavior. | UP | 014H, 035 |
| [#190](https://github.com/opengeospatial/ogcapi-connected-systems/issues/190) | Open | Multiple-encoding direction favors advertised channel selection through AsyncAPI, but the final model is unresolved. | UP | 012, 014H, 035 |
| [#191](https://github.com/opengeospatial/ogcapi-connected-systems/issues/191) | Open | Recorded proposal renames legacy `consys` CloudEvent types to `csapi`; the pinned draft still uses `org.ogc.api.consys.*`, so no adopted change is present. | PCD | 014H, 035 |
| [#192](https://github.com/opengeospatial/ogcapi-connected-systems/issues/192) | Open | [PR #198](https://github.com/opengeospatial/ogcapi-connected-systems/pull/198) merged to the Part 3 branch as commit [`c95c1d60`](https://github.com/opengeospatial/ogcapi-connected-systems/commit/c95c1d6003359d0883c4dc759d7a148ab115fdb1), aligning draft Resource and Batch Event identifiers with CloudEvents `source`+`id` uniqueness instead of UUID-only values. The issue remains open and the change is not in an approved Part 3 publication. | PCD | 014H, 035 |
| [#193](https://github.com/opengeospatial/ogcapi-connected-systems/issues/193) | Open | Aggregate collection-summary events are distinct from HTTP bulk and CloudEvents batch format. Direct branch commit [`6f529a15`](https://github.com/opengeospatial/ogcapi-connected-systems/commit/6f529a15bfa63259febc3620378d3e5a06305333) makes them an optional subclass of Resource Events, renames the class to `/req/resource-events/batch-events`, and states their high-volume summary purpose; the issue remains open and the change is not an approved publication. | PCD | 014H, 035 |
| [#194](https://github.com/opengeospatial/ogcapi-connected-systems/issues/194) | Open | `parentID` purpose, scope, naming, requirement level, and URL-versus-local-ID representation remain unresolved. | UP | 014H, 035 |
| [#195](https://github.com/opengeospatial/ogcapi-connected-systems/issues/195) | Open | Event payload options and boundary from native Resource Data Messages remain unresolved. | UP | 012, 014H, 035 |
| [#200](https://github.com/opengeospatial/ogcapi-connected-systems/issues/200) | Open | The official Connected Systems overview advertises Part 1 and Part 2 ReDoc examples whose deployment-relative URLs return 404; the defect was reported August 20, 2026 and has no disposition. | UP | 009, 014, 046 |
...
| IDR-SRV-013 | Error/status behavior, direct `400`/`409` obligations, cascade and PATCH conflicts, the Deployment `recursive`/false-`400` risk, write-format/validation ambiguity, draft CRUD dependency, and bulk/partial-failure gaps: #23, #61, #141, #164, #166, #169-#171, #178, #181, #185. |
| IDR-SRV-014 | Modular and bundled OAS provenance, dependency closure, tool behavior, examples, omitted or contradictory operations/parameters/endpoints, later corrections, and broken rendered-documentation publication: #12, #13, #48, #57, #77-#79, #81, #114, #137, #146, #148, #169-#170, #177, #185-#186, #200. |
...
- IDR-SRV-014 must analyze both tagged modular OAS trees and both released bundled OAS 3.1 files. It must account for known reference, omission, recursion/tooling, example, post-release maintenance, and broken rendered-publication evidence rather than treating one rendering as the whole API contract.
...
- IDR-SRV-014H must establish the exact draft Part 3 authority/completeness baseline, compare active CS-Go and OSH implementations, and identify early downstream constraints without selecting the Glaux streaming architecture or promoting either implementation's topic layout into a requirement.
- IDR-SRV-035 must consume accepted IDR-SRV-014H, refresh material deltas, and make the final Glaux streaming architecture and Part 3 adoption/profile decision. MQTT, transport classes, CloudEvents details, multiple encodings, batch events, parent identifiers, and event payload boundaries are not a published CSAPI 1.0 server obligation.
...
|---|---|---|---|
| September 15, 2026 | 1.12 | Refreshed the final streaming/Part 3 decision evidence for IDR-SRV-035; advanced the Part 3 pin from `c95c1d60` to `6f529a15`; recorded Batch Resource Events' new optional Resource Events subclass and requirement identifiers plus removal of the generated HTML; reconfirmed the missing MQTT binding, ATS, and Part 3 AsyncAPI; reconfirmed issues #14, #68, and #187–#195 remained open, CS-Go's released Pub/Sub behavior had no material later change, and OSH PR #194 remained an unchanged draft; and recorded the in-review report's disabled-by-default experimental Glaux profile recommendation pending project-lead acceptance. | Glaux research workflow |
