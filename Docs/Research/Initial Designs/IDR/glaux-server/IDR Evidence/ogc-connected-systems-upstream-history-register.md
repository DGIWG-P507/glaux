...
`Open` and `Closed` below are only the states observed through the latest bounded refresh on August 31, 2026. A closed issue is not proof that its proposed change was implemented; issue [#64](https://github.com/opengeospatial/ogcapi-connected-systems/issues/64) is a concrete counterexample.
...
| Part 3 and AsyncAPI | Part 2 assigns publish/subscribe binding to a future Part 3. A legacy AsyncAPI 2.6 support file is packaged with Part 2. The active [`part3-working-draft` tree](https://github.com/opengeospatial/ogcapi-connected-systems/tree/c95c1d6003359d0883c4dc759d7a148ab115fdb1/api/part3) at commit `c95c1d60` contains draft Resource Events, Batch Resource Events, Resource Data Messages, and content-negotiation material but no AsyncAPI file. Its source entry marks the document `swg-draft` and explicitly leaves the MQTT clause unwired, the ATS absent, and examples/relationship annexes missing. | Research authority, completeness, and implementation divergence early in IDR-SRV-014H, then use that accepted baseline plus material deltas for the final architecture/adoption-profile decision in IDR-SRV-035. Label pre-publication support as an exact draft compatibility profile or project extension, not approved OGC conformance. Reserve “non-conformance” for an actual unmet applicable requirement. |
...
| [#193](https://github.com/opengeospatial/ogcapi-connected-systems/issues/193) | Open | Aggregate collection-summary events are distinct from HTTP bulk and CloudEvents batch format; semantics remain undecided. | UP | 014H, 035 |
