...
**Checkpoint:** September 19, 2026, through Pass 3c iteration 21 (an IDR-030 coverage correction, then queue batch 3, the committed deep read of IDR-034). Iteration 20 had recorded IDR-030 as fully read while its own evidence report disclosed that seven sections were only skimmed; iteration 21 corrected the entry before reading, finished those sections, and restored the entry on that basis. Finishing them sharpened the F-19 restore instance and supplied two ready-made fixtures. IDR-034 then confirmed an instance of F-14(c), the unstated observation ordering direction at Guide line 444. The queue stands at **27 batches, 3 done and 24 remaining**, recorded in `batch_queue` of [review-state.json](review-state.json).
...
| Research | Synthesis/register read; full reads of 008, 011, 029, 030, 031, 034, 036, 037, 038, 039, 040, 041, 042, 043, 050, 052; six partial reads with recorded evidence | Key-section screen of the 47 reports whose read depth was never recorded plus the unread sections of the six partial reports (queue batches 6-14); two committed deep reads outstanding, IDR-039A and IDR-055 (queue batches 4-5) |
...
**Third corroborating source (iteration 21, IDR-034).** IDR-SRV-034 Section 13.4 closes with "Principal-specific results use partitioned/private caches." The instance now rests on three accepted reports: IDR-040 Section 10.4, IDR-042 Section 7 line 341 and IDR-034 Section 13.4. Its substance and recommended fix are unchanged. Evidence: [26-pass-3c-21-idr030-completion-and-idr034.md](evidence/26-pass-3c-21-idr030-completion-and-idr034.md), Section 3.3.
...
Remaining research deep-read scope: two committed deep reads, IDR-039A and IDR-055, not yet begun, queued as batches 4-5.
