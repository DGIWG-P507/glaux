...
**Checkpoint:** September 19, 2026, through Pass 3c iteration 17 (research-deep-read-remainders slice 2 closed: IDR-040 Sections 9-21 fully read and accounted against Guide v1.3. Its invariants are substantially adopted and its typed policy machinery is a recorded scope choice, but one evidenced gap was found and recorded under F-03: no Guide rule binds a response cache validator or cache key to the authorized view).
...
| Research | Synthesis/register read; full reads of 008, 011, 029, 031, 036, 037, 038, 039, 040, 041, 050, 052; other partial reads | Corpus-wide key-section sweep and several previously promised deep reads |
...
Research portions: IDR-008 sections 16-19, IDR-037 appendices A-D, IDR-038 appendices A-C, and IDR-039 section 7 and sections 21-22 were closed as slice 1 in iteration 16; IDR-040 sections 9-21 were closed as slice 2 in iteration 17. Those five reports are now fully read. Remaining: IDR-042's unread sections (3-4, 6, 7, 9, 10, 15, 16, 19-20, the next selected slice) and then IDR-043's unread sections. Additional promised deep reads were 030, 034, 039A and 055, none yet begun. Some CS-GO/OSH pinned-source spot checks remain unaccounted for.
...
**Next consideration for that instance:** State in Section 4.10 or Section 6 that validators and any cache key are bound to the authorized view, and that authorization-dependent representations carry `Cache-Control: private` (or `no-store`). Add the cache/validator channel to the line 1019 disclosure-channel test list. Both are documentation and test changes within existing owners; no new capability, policy engine or caching layer is implied, and the choice between `private` and `no-store` remains the owner's.
...
Remaining research deep-read scope: IDR-042's unread sections (next selected slice), then IDR-043's unread sections, and the committed deep reads IDR-030, IDR-034, IDR-039A and IDR-055, none yet begun.
