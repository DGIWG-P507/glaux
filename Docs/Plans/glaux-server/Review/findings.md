...
**Checkpoint:** September 19, 2026, through Pass 3c iteration 19 (research-deep-read-remainders slice 4 closed: IDR-043's unread sections fully read, substantially adopted, with a confirmed F-17 instance. All four partial-report remainder slices are now closed. The same iteration mapped every remaining finish condition into a finite numbered queue of **30 batches, 1 done and 29 remaining**, recorded in `batch_queue` of [review-state.json](review-state.json)).
...
| Research | Synthesis/register read; full reads of 008, 011, 029, 031, 036, 037, 038, 039, 040, 041, 042, 043, 050, 052; other partial reads | Corpus-wide key-section sweep (47 reports, queue batches 6-14) and the four committed deep reads (queue batches 2-5) |
...
Research portions: all four partial-report remainder slices are closed. IDR-008 sections 16-19, IDR-037 appendices A-D, IDR-038 appendices A-C and IDR-039 section 7 and sections 21-22 closed as slice 1 in iteration 16; IDR-040 sections 9-21 as slice 2 in iteration 17; IDR-042's unread sections as slice 3 in iteration 18; IDR-043's unread sections as slice 4 in iteration 19. Those seven reports are now fully read. Remaining research work: the four committed deep reads 030, 034, 039A and 055, none yet begun (queue batches 2-5); the key-section screen of the 47 reports whose read depth was never recorded (queue batches 6-14); and the CS-GO/OSH pinned-source spot checks, merged into queue batch 8.

**The complete remaining queue.** Every remaining finish condition across all six areas is now mapped to a finite numbered batch in `batch_queue` of [review-state.json](review-state.json): **30 batches, 1 done and 29 remaining.** The queue separates genuinely unreviewed work from coverage that was simply never recorded, and from bookkeeping already corrected in iteration 16. Its sizing is an estimate based on what iterations 16-19 absorbed, with five recorded uncertainties; it is not a completion guarantee.
...
Remaining research deep-read scope: the four committed deep reads IDR-030, IDR-034, IDR-039A and IDR-055, none yet begun, queued as batches 2-5.
