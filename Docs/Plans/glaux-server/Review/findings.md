...
**Checkpoint:** September 19, 2026, through Pass 3c iteration 20 (queue batch 2, the committed deep read of IDR-030: its core lifecycle posture is adopted at Guide line 527, its lifecycle machinery is a recorded scope choice, and one documentation gap is recorded under F-19 for restore versus deletion. The same iteration corrected two queue-accounting problems, replacing sample-based leaf-to-issue fidelity with a complete comparison of all 286 issues and accounting for the six partial reports inside existing batches. The queue now stands at **27 batches, 2 done and 25 remaining**, recorded in `batch_queue` of [review-state.json](review-state.json)).
...
Remaining research deep-read scope: three committed deep reads, IDR-034, IDR-039A and IDR-055, none yet begun, queued as batches 3-5.
