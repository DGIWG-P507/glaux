...
**Status: the first six approved bounded updates are delivered. The seventh adopts per-relation spelling/comparison rules in Guide v1.9; its four issue amendments are pending publication. Software implementation remains pending; other actions remain proposed.**
...
| Owner | Stage-specific propagation / current status |
|---|---|
| [#56 — association navigation](https://github.com/DGIWG-P507/glaux-server/issues/56) | Independent applicable edge/relation/target expectations, exact Part 1 output, relation-only comparison and named adaptations. No later event/MQTT/filtering implementation. **Amendment pending.** |
| [#212 — native System Events](https://github.com/DGIWG-P507/glaux-server/issues/212) | Preserve the qualified `system` JSON mapping; reject a wrong relation on a correct parent in the fixture without claiming OGC settled it. **Amendment pending.** |
| [#214 — AsyncAPI](https://github.com/DGIWG-P507/glaux-server/issues/214) | Preserve the exact Glaux URN and independent wrong-relation/right-target check. **Amendment pending.** |
| [#228 — queryables](https://github.com/DGIWG-P507/glaux-server/issues/228) | Preserve the full URI in JSON/header output and its scoped target; distinguish selected-output checks from claims about allowed CURIE alternatives. **Amendment pending.** |

**Boundary:** a spelling/interpretation clarification, not new navigation capabilities, a vocabulary service, a compatibility subsystem or an upstream standards amendment. Existing original bodies/pins, all 286 leaves/dependencies, Goal v1.8 and the completed review remain unchanged. Publication/readback evidence will follow delivery. No licence/settings change, installation or coding is authorised.
...
**Decision record:** licence remains unselected; enforcement changes remain unauthorised. The first six bounded updates are delivered; the seventh adopts only F-11's per-relation spelling/comparison clarification. This does not convert the remaining recommendations into requirements.
...
**Current handoff:** finish the authorised F-11 update by publishing and verifying the four issue amendments, record delivery, then stop. No other proposal, coding or settings change is authorised by this update. Licence/enforcement decisions remain outstanding. No implementation issue is complete. The closed `review-state.json` remains review coverage, not an implementation queue.
