...
**Status: the first five approved bounded updates are delivered. The post-backup-deletion clarification is adopted in Guide v1.8; its five stage-specific issue amendments are being published. Software implementation remains pending; other actions remain proposed.**
...
|---|---|
| [#26 — first System restore](https://github.com/DGIWG-P507/glaux-server/issues/26) | State the backup/history limit and keep inspection isolated. Do not pull later DELETE, export or activation machinery into the first System slice. **Amendment pending.** |
| [#126 — observation/event restore](https://github.com/DGIWG-P507/glaux-server/issues/126) | Add later-deletion/evidence-gap fixtures for implemented families; separate backup comparison from reconciled query results, with no later exchange/cutover implementation. **Amendment pending.** |
| [#251 — complete-model restore](https://github.com/DGIWG-P507/glaux-server/issues/251) | Extend both evidence branches across the implemented model and inspect held reads/export without activating production service. **Amendment pending.** |
| [#252 — epochs/fencing](https://github.com/DGIWG-P507/glaux-server/issues/252) | Make deletion/history disposition an additional activation prerequisite; scope the recorded limited-view decision and incomplete export. Existing epoch/fencing/command guards remain independent. **Amendment pending.** |
| [#283 — candidate regression](https://github.com/DGIWG-P507/glaux-server/issues/283) | Recheck reads and re-export independently against the candidate, retaining audit and no-actuation checks. **Amendment pending.** |

**Boundary:** no external deletion ledger, archive framework, failover service or fabricated missing history. This adopts the research's reconcile-before-serving principle but explicitly narrows its stronger archive/ledger guarantee; the limited-view decision is a Glaux recovery choice, not an OGC obligation. Earlier audit amendments are retained and their statement that this proposal was then unadopted is historical, superseded only on this point by the new amendment. Goal v1.8, all 286 task definitions/dependencies and the completed review remain unchanged. No licence/settings change, installation or coding is authorised. Publication/readback evidence will follow delivery.
...
**Decision record:** licence remains unselected; enforcement changes remain unauthorised. The first five bounded updates are approved and delivered; the sixth adopts only F-19's separate restore/deletion clarification. This does not convert the remaining recommendations into requirements.
...
| F-19 — post-backup deletions | **Adopted in Guide v1.8:** reconcile available authoritative deletion evidence before normal reads/export; hold unresolved scope by default. A policy-permitted recorded limited-view decision cannot waive known deletions/other restore guards or claim complete history. No external deletion ledger. | Guide §4.7/scenario 6; #26/#126/#251–#252/#283. **Guide adopted; issue publication pending; implementation pending.** Separate from captured audit survival. |
...
**Current handoff:** finish the authorised restore/deletion update by publishing and verifying its five issue amendments, then record delivery and stop. No other proposal, coding or settings change is authorised by this update. Licence/enforcement decisions remain outstanding. No implementation issue is complete. The closed `review-state.json` remains review coverage, not an implementation queue.
