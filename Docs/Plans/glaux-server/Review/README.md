...
**Two findings were reconsidered for escalation and neither was escalated.** The audit-boundary finding now has complete evidence - the task that restores everything and the task that implements retention both list every other kind of evidence and leave audit out - and the Guide’s own lists of what a restore re-establishes and what a retention policy may remove do not mention audit either. So what the evidence shows is a question nobody has answered rather than a rule anyone has broken, and raising the severity would turn an honest silence into an accusation the evidence does not support. **Note that this is about audit surviving a restore or a retention window, which is a different question from whether audit should be tamper-proof.** The Guide does decline to require a tamper-proof ledger, but that declines the second question, not the first; the final assessment carries an erratum correcting a passage that ran the two together. The related export-accountability finding keeps its severity for the simpler reason that nothing has been built yet. What both gained is a concrete place and time to be decided.
