...
The last substantive reviewer response is **Copilot Pass 3c, iteration 14** (standards batch: the nine remaining IDR-011 Section 14.3 abstract-test rows, all confirmed against the published 23-001/23-002 Annex A text, and the Part 2 Annex A.1 `api-common` inheritance question, closed as supported; see the "Resolved standards checks" table in findings.md). The record has **22 finding IDs, including two withdrawals**, with subsequent qualifications preserved; iterations 12, 13 and 14 each added a related abstract-test instance under F-08 and no new finding number.

The next selected batch is the F-13 controlling extension-rule check (`observation-extension-policy`): whether the published texts impose or permit an extension-member rule for inbound Observation JSON and how association-style `@id`/`@link` members are distinguished. All six carried standards groups are closed. The machine state is authoritative for that cursor as review continues.
...
| 2. Standards | Largely complete; all six carried groups closed (SWE quality, array flags, Features Part 3/CQL2 identifiers, SensorML class identifiers, IDR-011 Section 14.3 abstract-test rows, Part 2 Annex A.1 inheritance) | Resolve the F-13 extension question (`observation-extension-policy`) with evidence or a stated bounded unresolved result |
