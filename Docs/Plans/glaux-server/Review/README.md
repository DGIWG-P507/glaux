...
The last substantive reviewer response is **Pass 3c, iteration 21**, which began with a coverage correction. Iteration 20 had recorded IDR-030 as fully read while its own evidence report disclosed that seven sections were only skimmed. The state entry was corrected before any reading, those sections were then read in full, and the entry was restored on that basis. The archived report was preserved unchanged and the completion appended rather than rewritten.

Finishing that work paid off. It sharpened the **F-19** restore gap, where the Guide's restore section enumerates what a restore does not re-establish but omits deletions: the research turns out to specify an explicit restore sequence ending in reconciling the deletion ledger before publication, and it supplies two ready-made test fixtures. The iteration then executed queue batch 3, the committed deep read of IDR-034, which is substantially adopted and which confirmed an instance of **F-14**: the Guide fixes the observation sort key and tie-breaker at line 444 but never states the direction, so two conforming clients could page the same stream in opposite orders. The fix is one word.

Every remaining finish condition is mapped to a finite numbered queue held in `batch_queue` of [review-state.json](review-state.json): **27 batches, 3 done, 24 remaining.** It covers the committed deep reads, a key-section screen of the reports whose read depth was never recorded, the end-to-end scenario pass, the verification-quality pass, the issue backlog and the final assessment, and it separates genuinely unreviewed work from coverage that was simply never recorded and from bookkeeping already corrected.
...
The next selected batch is **batch 4 of 27**: the committed deep read of IDR-039A. The machine state is authoritative for that cursor as review continues.
...
| 3. Research and peer evidence | Partially complete; all deep-read remainder slices closed, IDR-030 and IDR-034 completed (16 reports now fully read); remaining work queued as batches 4-14 | Account for key findings/recommendations/open questions across 71 reports, finish the two remaining committed deep reads (IDR-039A, 055) and the pinned CS-GO/OSH checks; not blanket full reads of every plan/report |
