...
**Status:** Baselined — early review-follow-up test and audit clarifications adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Adopts the first bounded review-follow-up changes authorised by the project lead's September 20, 2026 `proceed`: independent interpretation of actual HTTP responses, explicit audit permissions and bounded denial recording, and audit preservation at the backup recovery point. These are Glaux design/test choices within Goal v1.8, not new standards obligations. Roadmap v1.19 aligns the current Guide reference; issues #15, #19, #22 and #26 receive traceable amendments without erasing their preparation pins. Later issue amendments remain tracked in the [review action checklist](Review/action-list.md). No new capability, framework, licence choice, repository-setting change or executed software verification is included.
