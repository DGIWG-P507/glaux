...
**Status:** Baselined — approved review-follow-up clarifications through ControlStream admission adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the project lead's next bounded September 20, 2026 `proceed` for F-18: derive ControlStream `live` from command-admission capability, distinguish false/unknown admission, and assess Feasibility separately. Sections 4.6/4.9/6.4/8/13 and Roadmap v1.28 align existing #156/#159/#160/#163/#164/#177/#180. These are labelled Glaux choices where the published standard is silent; no new lifecycle, device-readiness score or automatic cancellation is introduced. Goal v1.8, architecture, task sequence and prior approved choices remain unchanged. No research expansion, licence/settings change or coding is included; delivery is tracked in the [review action checklist](Review/action-list.md).
...
The Guide remains baselined. Issue publication is complete; the first implementation iteration is [task 1.1.1 / issue #3](https://github.com/DGIWG-P507/glaux-server/issues/3), the approved-prerequisite inspection described in [Roadmap §5.3](glaux-server-roadmap.md#53-immediate-next-step), without implicit installation. Follow the [action checklist](Review/action-list.md) for the current bounded documentation handoff; this update does not execute that issue. Continue to push and summarize one completed iteration at a time; implementation proceeds one ready issue per authorized iteration.
