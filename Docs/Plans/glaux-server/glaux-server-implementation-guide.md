...
**Status:** Baselined — approved test, audit, cache and observation-order clarifications adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Adopts the project lead's next bounded September 20, 2026 `proceed`: bind cache reuse and validators to the authorised representation, default protected responses to `private, no-store`, and state ascending observation result-time/ID order explicitly. These resolve the F-03 cache instance and F-14(c) at the design/instruction level, not through executed software. Roadmap v1.20 aligns the current Guide reference; issues #151/#264 and #113/#233 receive traceable amendments while preserving their preparation pins. The earlier response-test/audit propagation is complete in its nine named issues; delivery and remaining proposals are in the [review action checklist](Review/action-list.md). Goal v1.8 and the existing architecture, paging model and task sequence are unchanged. No new cache service, licence choice, repository-setting change or implementation is included.
