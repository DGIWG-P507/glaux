...
**Status:** Baselined — technical follow-up questions addressed; owner decisions remain; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the next bounded September 21, 2026 `proceed` for fq-13. Sections 4.2/4.9/13 state the published System type/kind mappings and qualify Glaux's Command issue-time receipt/ownership choice. Eight existing issues carry stage-specific mapping, admission, mutation and query fixtures; no new model, subsystem or task is added. Roadmap v1.34 aligns the current reference. Goal v1.8, architecture and all task definitions/dependencies are unchanged. The action list's technical questions are addressed; licence/enforcement decisions, optional cleanup and implementation proofs remain separate. No coding, installation, settings change or reopened review is authorised. Delivery is tracked in the [review action checklist](Review/action-list.md).
...
Media types are `application/geo+json`, `application/sml+json`, `application/json`, `application/swe+json`, `application/swe+text`, and `application/swe+binary` for their applicable resource/operation combinations. Text is not silently renamed CSV. The separate SensorML draft-note and SWE vendor-prefixed treatments are in §13.
...
The Guide remains baselined. Issue publication is complete; when explicitly authorised, the first implementation iteration is [task 1.1.1 / issue #3](https://github.com/DGIWG-P507/glaux-server/issues/3), the approved-prerequisite inspection described in [Roadmap §5.3](glaux-server-roadmap.md#53-immediate-next-step), without implicit installation. The [action checklist](Review/action-list.md) controls the current review-follow-up handoff; its remaining actions are not closed by completing one group of amendments. This update does not execute #3. Continue to push and summarize one completed iteration at a time; implementation proceeds one ready issue per authorized iteration.
