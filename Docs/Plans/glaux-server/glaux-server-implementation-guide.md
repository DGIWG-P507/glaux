...
**Status:** Baselined — approved review-follow-up clarifications through diagnostics access adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Adopts the project lead's next bounded September 20, 2026 `proceed`: make metrics and enabled traces internal or access-restricted by default, distinct from minimal public liveness (review question fq-09). Section 4.12 specifies the access boundary and its negative/positive verification. Roadmap v1.22 aligns the reference; existing issue #256 receives a pinned amendment without erasing its original instructions. Goal v1.8, architecture and task sequence are unchanged. No new tracing/backend requirement, administration service, licence choice, repository-setting change or implementation is included. Delivery and remaining proposals are in the [review action checklist](Review/action-list.md).
