...
**Status:** Baselined — review follow-up continues; Part 2 collection exposure selected; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the next bounded September 21, 2026 `proceed` for fq-06. Section 4.1.2 selects one default collection for each of the six top-level Part 2 families, following IDR-010's generic-client discovery recommendation. Sections 6.2/7.2.1/13 and Roadmap v1.31 carry exact markers, staged implementation and actual collection-test applicability into existing owners. Goal v1.8, architecture and all task definitions/dependencies are unchanged. Three other technical questions remain open; no coding, installation, settings change or reopened review is authorised. Delivery is tracked in the [review action checklist](Review/action-list.md).
