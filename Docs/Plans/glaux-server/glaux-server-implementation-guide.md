...
**Revision summary:** Records the project lead's next bounded September 20, 2026 `proceed` for F-07: explain the existing latest-observation interpretation and client limits, with discriminating scope examples and a supplemental-test qualification. Sections 4.4/8/13 and Roadmap v1.25 align the existing #112 instructions. The authorised, filtered endpoint-wide maximum and retained ties are unchanged, as are Goal v1.8, architecture, task sequence and prior approved changes. No per-series aggregate, peer-behaviour claim, upstream filing, licence/settings choice or implementation is included. Delivery and remaining proposals are in the [review action checklist](Review/action-list.md).
