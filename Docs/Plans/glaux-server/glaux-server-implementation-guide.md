...
**Status:** Baselined — approved review-follow-up clarifications through link-relation spelling adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Adopts the project lead's next bounded September 20, 2026 `proceed` for F-11: state the per-relation output and comparison rules, distinguish published spelling conflicts from the unresolved System Event mapping, and align the existing navigation fixtures. Sections 4.1.1/4.5/8/13 and Roadmap v1.24 guide amendments to #56/#212/#214/#228. Goal v1.8, architecture, task sequence and prior approved changes remain unchanged. No universal prefix rewrite, new compatibility subsystem, licence choice, repository-setting change or implementation is included. Delivery and remaining proposals are in the [review action checklist](Review/action-list.md).
