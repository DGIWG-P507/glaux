...
**Status:** Draft — iteration 2 complete; whole-guide review in iteration 3 next<br>
**Depends On:** [Glaux Server Goal and Definition v1.7](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Completes drafting iteration 2: specifies write/patch, command recovery, query mappings, optional delivery and administrative exchange contracts; incorporates the agreed provenance/quality/disclosure clarifications; and records Part 5 implementation as deferred. Goal v1.7 remains unchanged. These are design decisions and tests to implement, not claims of tested software. Iteration 3 will review the whole guide and finish it for Roadmap development.
...
SSE `id` is an opaque authenticated-encrypted cursor binding position, caller, exact scope, policy/mapping version and expiry. Accept it only through `Last-Event-ID`; no unauthenticated plaintext sequence or bearer credential is embedded. Preserve replay order within a subscriber, and advance a cursor only after all eligible earlier events have been written to that stream. This is not proof that the application processed them. Malformed/tampered or mismatched-scope cursors receive `400`; expired/retired recovery context receives `410` before streaming, with fresh-snapshot guidance; revoked authority receives the ordinary denial. Recheck authority during delivery and close on expiry/revocation. Bound subscriber buffers and disconnect slow consumers so they can resume or resnapshot. SSE remains a Glaux interface, not a Part 3 binding.
...
For synchronous work, retain a private durable admission/attempt record before dispatch, then publish the Command/Feasibility and its terminal status/results atomically when the authoritative outcome is known. Do not expose temporary `PENDING` history or hold a database transaction across execution. Distinguish the HTTP waiting budget from the work's execution deadline. Disconnect or waiting-budget exhaustion neither cancels admitted work nor proves physical failure. If the server unexpectedly cannot obtain the synchronous outcome within its bound, return a safe `500` problem when still able to respond; use `504` only when actually acting as an upstream gateway. Retain the admission for reconciliation, describing uncertainty without a fabricated CommandStatus or rollback promise. `503` remains an overload/unavailability response, not an execution-status substitute. [RFC 9110 §15.6](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.6)

Serialize admission and conflicting ControlStream/parent mutations using the same application transaction boundary. Outstanding private admissions protect their pinned parent/contract just as accepted public work does: reject conflicting deletion or incompatible schema changes with the documented state conflict while unresolved work depends on them. If deletion commits first, admission cannot succeed or dispatch. Reconciliation never recreates a previously deleted public parent/resource. Test the race through terminal publication, not only the pre-dispatch check.

A same-key, same-intent retry reauthorizes disclosure and rejoins existing work within its own wait budget or returns the recorded completed outcome; it never dispatches again just because a response was lost. Do not expire/recycle its key while admitted work is unresolved; bounded retention applies after terminal resolution. Once reconciled, publish exactly one terminal status. Conflicting late callbacks remain restricted evidence, not a transition from `FAILED` to `COMPLETED`.
...
- **Writes/schema discovery:** ignored body-local IDs versus protected UID/parent/schema changes; omitted-field removal on PUT; lossless versus rejected lossy replacement; Merge Patch null/array behavior and untouched richer content; failed full-resource validation leaves every representation unchanged; omitted/unsupported `cmdFormat`, missing `obsFormat` and unacceptable wrapper media are distinct cases.
- **Command recovery:** exact `201`/Location/Content-Location/body; durable admission, external effect, terminal publication and lost-response crash boundaries; concurrent same-key retries never create another dispatch; a timeout followed by completion creates one terminal report; unkeyed ambiguity makes no deduplication promise; successful feasibility analysis can answer NO without actuation.
...
### 9.2 Second-pass dispositions and implementation checks

The first-draft questions now have the following dispositions. Selecting a contract does not establish interoperability or conformance; remaining executable proofs belong to implementation. Iteration 3 reviews whether these choices work together, without reopening the entire research program.
...
| Dependencies/schema/time | Retain the small Rust stack, offline resolution and exact comparisons; no software installed or versions falsely verified | Compile/pin versions and features, prove recursive schemas and exact time representation including source precision, calendar/offset/leap-second cases; reject unsupported precision explicitly, never silently round |
...
| Recovery and limits | Versioned bounded manifest, source epoch/revision/predecessor, scoped snapshots and explicit conflict operations (§4.11) | Generate schema/fixtures; prove snapshot/catch-up ordering, enforce finite limits, measure reference values and publish the configured recovery window |
...
Version 0.1 was drafting iteration 1. Version 0.2 incorporated the approved Part 4/querying scope in Goal v1.7. Version 0.3 completes drafting iteration 2 after the project lead's `proceed` on the combined provenance/Part 5 discussion: keep Goal v1.7, incorporate focused provenance clarifications, and defer Part 5 implementation while preserving codec separation. It also disposes the first-draft technical questions in §9.2. Research reports and synthesis addenda remain the historical evidence, not retroactively rewritten scope decisions.

The next `proceed` authorizes iteration 3's whole-guide/workflow review and finalization for Roadmap development. This guide is not yet baselined and no server software is claimed implemented. Push each completed iteration and summarize concrete outcomes; no new document or approval process is required. Do not start the Roadmap until the guide is baselined under the existing planning guidance.
