...
**Status:** Baselined — approved review-follow-up clarifications through export accountability adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Adopts the project lead's next bounded September 20, 2026 `proceed`: clarify that resource exchange does not replicate the source server's complete authenticated audit trail (F-21), and require scoped local audit records for export generation and authorised release attempts/outcomes (F-22). A local handoff does not prove recipient receipt; partial or uncertain output remains explicit. Roadmap v1.21 aligns the current Guide reference; existing issues #239/#240/#287 receive traceable amendments while preserving their original bodies and preparation pins. Goal v1.8, the manifest format, architecture and task sequence are unchanged. No audit-replication service, receipt platform, licence choice, repository-setting change or implementation is included. Delivery and remaining proposals are in the [review action checklist](Review/action-list.md).
