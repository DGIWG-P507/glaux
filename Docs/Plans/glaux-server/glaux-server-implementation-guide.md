...
**Status:** Baselined — approved review-follow-up clarifications through observation unknown-member handling adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the project lead's next bounded September 20, 2026 `proceed` for F-13: ordinary observation JSON ignores unmapped, unadvertised top-level members consistently on POST/PUT/PATCH, without relaxing mapped-field or parent-contract validation. Sections 4.6/13 and Roadmap v1.27 align #98–#102, including the misleading published `foi@id` example. This is an explicit Glaux processing choice, not a universal extension rule or OGC correction. Goal v1.8, architecture, task sequence and prior approved choices remain unchanged. No new research, licence/settings change or coding is included; delivery is tracked in the [review action checklist](Review/action-list.md).
