...
**Status:** Baselined — approved review follow-up continues; SWE validation-entry and Quantity-label choices adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the project lead's next bounded September 20, 2026 `proceed` for fq-01/fq-02: use the published named encoding definitions rather than treating the incomplete encoding root as a universal validator, and retain the JSON Quantity schema's required label. Sections 4.3/13 and Roadmap v1.29 align existing #7/#8/#28/#128/#140. Preserve upstream bytes, complete wrapper validation and separate semantic/codec proofs. Goal v1.8, architecture, task sequence and prior choices are unchanged. The review-action follow-up remains in progress; this update does not authorise coding or settle the other six technical questions. Delivery is tracked in the [review action checklist](Review/action-list.md).
