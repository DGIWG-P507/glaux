...
**Status:** Baselined — review follow-up continues; SensorML media-type boundary clarified; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the next bounded September 21, 2026 `proceed` for fq-04. Sections 6.2/13 distinguish the published SensorML media type from its retained draft note and explicitly select no vendor-prefixed SensorML alias in this baseline. Existing #83/#85/#86 carry media-contract tests and honest client-subset evidence; no compatibility feature or new task is added. Roadmap v1.32 aligns the current reference. Goal v1.8, architecture and all task definitions/dependencies are unchanged. Two technical questions remain open; no coding, installation, settings change or reopened review is authorised. Delivery is tracked in the [review action checklist](Review/action-list.md).
