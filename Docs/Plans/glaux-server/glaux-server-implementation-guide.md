...
**Status:** Baselined — approved review-follow-up clarifications through restore/deletion limits adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Adopts the project lead's next bounded September 20, 2026 `proceed`: state the effect of restoring before a later deletion, reconcile available authoritative deletion evidence before serving/re-export, and make missing-history limitations and operator decisions explicit (F-19's separate restore/deletion instance). Guide §§4.7/8 and Roadmap v1.23 align the instructions for existing issues #26/#126/#251/#252/#283. Original issue bodies and earlier amendments remain preserved. Goal v1.8, architecture, exchange format and task sequence are unchanged. No external deletion ledger, archive service, licence choice, repository-setting change or implementation is included. Delivery and remaining proposals are in the [review action checklist](Review/action-list.md).
