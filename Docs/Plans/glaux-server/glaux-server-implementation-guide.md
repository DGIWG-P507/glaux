...
**Status:** Baselined — review follow-up continues; spatial-literal and published-test clarifications adopted; implementation not started<br>
**Depends On:** [Glaux Server Goal and Definition v1.8](glaux-server-goal-and-definition.md), Approved

**Revision summary:** Records the project lead's bounded September 21, 2026 `proceed` for fq-03/fq-05: explicitly test LineString members in GeometryCollection, and account for the five recorded published-test discrepancies, including the now-verified A.115 advertising error. Sections 7.2.3/7.4/13 and Roadmap v1.30 carry source-preserving adaptations and supplemental evidence into existing tasks. Goal v1.8, architecture and all task definitions/dependencies are unchanged. Four other technical questions remain open; no implementation, upstream filing or new review is authorised. Delivery is tracked in the [review action checklist](Review/action-list.md).
...
Part 2 Common abstract test A.1 supplies no separate executable test. Its evidence includes the inherited Part 1 results and [A.2's collection checks](https://docs.ogc.org/is/23-002/23-002.html#_conf_api-common_resource-collection), which apply eighteen Features Part 1 tests to each exposed collection whose `itemType` is not `feature`. Record the actual candidate's exposed collections and applicability. No qualifying collection means that conditional check is inapplicable, not passed; absent later-phase capabilities remain incomplete, not inapplicable merely because they have not been built. This does not select which Part 2 collections Glaux exposes or waive that decision before the final declaration check.
...
Sources: [Part 2 requirements 31/61/115 and corresponding Annex tests][S2]; [Part 1 A.66 / Recommendation 5][S1]. A.115's candidate is now confirmed; this does not settle fq-06's choice of which Part 2 collections to expose.
